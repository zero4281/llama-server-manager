...
- While in this headless state, `print_message` (Section 10.8) falls back to writing text directly via `sys.stdout`/`sys.stderr` (never the `print()` builtin) instead of rendering through curses. This fallback is internal to `UIManager`; callers always call `print_message` the same way regardless of whether curses is active.
